...
using namespace custom_plugins;

/// \brief Static counter for generating unique scene names across Sensors instances
static std::atomic<uint32_t> g_sensorInstanceCounter{0};

/// \brief Static mutex to protect resource creation operations (sensor creation/removal, renderUtil init)
/// This ensures thread safety when multiple Sensors instances operate concurrently
static std::mutex g_resourceCreationMutex;
...
// Private data class.
class gz::sim::systems::custom_plugins::SensorsPrivate
{
...
public:
  rendering::GlobalIlluminationVctPtr giVct;

  /// \brief GI VCT parameters passed to giVct
public:
  GiVctParameters giVctParameters;

  /// \brief Default GI data
public:
  GiDefaultData giDefaultData;
...
public:
  double ambientTemperatureGradient = -0.0065;
...
public:
  std::unordered_map<std::string, sensors::CameraSensor *> cameras;
...
public:
  std::unordered_map<Entity, sensors::SensorId> entityToIdMap;
...
public:
  std::chrono::steady_clock::duration updateTimeToApply;

  /// \brief Next sensors update time
public:
  std::chrono::steady_clock::duration nextUpdateTime;

  /// \brief Sensors to include in the next rendering iteration
public:
  std::set<sensors::SensorId> activeSensors;

  /// \brief Sensors to be updated next
public:
  std::set<sensors::SensorId> sensorsToUpdate;
...
public:
  void UpdateBatteryState(const EntityComponentManager &_ecm);
...
public:
  std::unordered_map<Entity, bool> modelBatteryState;

  /// \brief A map between model entity ids in the ECM to whether its battery
  /// state has changed.
  /// True means has charge, false means drained
public:
  std::unordered_map<Entity, bool> modelBatteryStateChanged;

  /// \brief A map of sensor ids to their active state
public:
  std::unordered_map<sensors::SensorId, bool> sensorStateChanged;

  /// \brief Disable sensors if parent model's battery is drained
  /// Affects sensors that are in nested models
public:
  bool disableOnDrainedBattery = false;

  /// \brief Mutex to protect access to sensorStateChanged
public:
  std::mutex sensorStateMutex;

  /// \brief VideoRecording manager
  public: custom::VideoRecordingManager videoRecorder;
};
...
void SensorsPrivate::WaitForInit()
{
  while (!this->initialized && this->running)
  {
    ...
    if (this->doInit)
    {
      ...
      {
        std::lock_guard<std::mutex> resourceLock(g_resourceCreationMutex);
      ...
      this->videoRecorder.SetScene(this->scene);
...
void SensorsPrivate::RunOnce()
{
  ...
  if (!activeSensorsEmpty || this->forceUpdate)
  {
      ...
      if (!this->giBuilt)
      {
        if (this->giVctParameters.enabled)
        {
          ...
          this->giVct->SetDebugVisualization(static_cast<
                                             rendering::GlobalIlluminationVct::DebugVisualizationMode>(this->giVctParameters.debugVisMode));
          this->giBuilt = true;
        }
      }
    ...
    if (this->updateTimeApplied <= this->updateTime)
    {
      ...
      this->videoRecorder.OnRender(this->updateTimeApplied);
    }
...
void SensorsPrivate::RenderThread()
{
  ...
  // Finalize any recordings still in flight so their videos are not lost,
  // and destroy the recorder cameras while the scene is still alive.
  this->videoRecorder.Shutdown();
...
void Sensors::RemoveSensor(const Entity &_entity)
{
  ...
  if (idIter != this->dataPtr->entityToIdMap.end())
  {
    ...
    for (auto &it : this->dataPtr->cameras)
    {
      ...
      if (this->conn_ptrs.find(name) != this->conn_ptrs.end())
      {
        this->conn_ptrs[name].reset();
      }
      if (this->cam_ptrs.find(name) != this->cam_ptrs.end())
      {
        this->cam_ptrs[name] = nullptr;
      }
      if (this->gpu_lidar_ptrs.find(name) != this->gpu_lidar_ptrs.end())
      {
        this->gpu_lidar_ptrs[name] = nullptr;
      }
      if (this->image_data_map.find(name) != this->image_data_map.end())
      {
        image_data_map[name] = gz::msgs::Image();
      }
      if (this->gpu_lidar_data_map.find(name) != this->gpu_lidar_data_map.end())
      {
        gpu_lidar_data_map[name] = LidarFrameView();
      }
    ...
    // Protect sensor removal with static mutex for thread safety across instances
    {
      std::lock_guard<std::mutex> resourceLock(g_resourceCreationMutex);
...
{
  math::Vector3d parsedValues = (_parentElem == nullptr) ? _defaultValue : _parentElem->Get<math::Vector3d>(_childName, _defaultValue).first;
                        ...
                        EventManager &_eventMgr)
{
  ...
  // Vulkan is required for multi-instance support
  // OpenGL has shared mesh cache issues in Ogre2 that cause "Mesh already exists" errors
  // However, users can override this by setting env var RENDER_ENGINE_API_BACKEND or through SDF, SDF gets priority in all cases
  const char *env_render_api = std::getenv("RENDER_ENGINE_API_BACKEND");
  std::string api_backend = (env_render_api == nullptr || env_render_api[0] == '\0') ? "vulkan" : env_render_api;
  std::string apiBackend = _sdf->Get<std::string>(
                                   "render_engine_api_backend", api_backend)
                               .first;
    ...
    else
    {
      ...
      if (giType == "vct")
      {
        this->dataPtr->giVctParameters.enabled = giElem->Get<bool>(
                                                           "enabled", this->dataPtr->giVctParameters.enabled)
                                                     .first;
  ...
  // Configure headless rendering based on environment and backend:
  // - Vulkan + EGL headless has issues with multiple instances ("Unable to setup EGL")
  // - Vulkan + GLX (display) works well for multiple instances
  // - OpenGL has mesh cache conflicts regardless of headless mode
  // Strategy: Use display (GLX) when available, fall back to headless (EGL) in containers
  const char *displayEnv = std::getenv("DISPLAY");
  bool hasDisplay = (displayEnv != nullptr && displayEnv[0] != '\0');

  if (hasDisplay)
  {
    // Display available - use GLX instead of EGL for better multi-instance support
    this->dataPtr->renderUtil.SetHeadlessRendering(false);
    gzdbg << "DISPLAY=" << displayEnv << " detected, using GLX rendering" << std::endl;
  }
  else
  {
    // No display (container) - must use EGL headless
    // Note: Multiple instances in the same container may still have EGL conflicts
    // Consider using separate containers or a virtual display (Xvfb) for multi-instance
    this->dataPtr->renderUtil.SetHeadlessRendering(true);
    gzdbg << "No DISPLAY detected, using EGL headless rendering" << std::endl;
  }

  // Generate a unique scene name for this Sensors instance to allow
  // multiple instances to coexist without visual name conflicts
  uint32_t instanceId = g_sensorInstanceCounter.fetch_add(1);
  std::string uniqueSceneName = "scene_" + std::to_string(instanceId);
  this->dataPtr->renderUtil.SetSceneName(uniqueSceneName);
  gzdbg << "Using unique scene name: " << uniqueSceneName << std::endl;
                         ...
                         const EntityComponentManager &_ecm)
{
  ...
  if (this->dataPtr->running && this->dataPtr->initialized)
  {
    ...
    if (hasRenderConnections ||
        ...
        this->dataPtr->forceUpdate ||
        this->dataPtr->videoRecorder.RequiresRender(_info.simTime) ||
        !sensorsWithPendingTriggers.empty())
    {
      ...
      this->dataPtr->forceUpdate =
          (this->dataPtr->renderUtil.PendingSensors() > 0) ||
          hasRenderConnections || this->dataPtr->videoRecorder.RequiresRender(_info.simTime);;
                                  ...
                                  const sdf::Sensor &_sdf, const std::string &_parentName)
{
  ...
  // Protect sensor creation with static mutex for thread safety across instances
  std::lock_guard<std::mutex> resourceLock(g_resourceCreationMutex);
  ...
  const std::string &sens_name = sensor->Name();
  this->conn_ptrs[sens_name] = nullptr;      // initialize to null
  this->cam_ptrs[sens_name] = nullptr;       // initialize to null
  this->gpu_lidar_ptrs[sens_name] = nullptr; // initialize to null

  //  Add to cam ptr or lidar ptr maps if applicable
  if (_sdf.Type() == sdf::SensorType::CAMERA ||
      _sdf.Type() == sdf::SensorType::DEPTH_CAMERA ||
      _sdf.Type() == sdf::SensorType::RGBD_CAMERA ||
      _sdf.Type() == sdf::SensorType::THERMAL_CAMERA ||
      _sdf.Type() == sdf::SensorType::BOUNDINGBOX_CAMERA ||
      _sdf.Type() == sdf::SensorType::SEGMENTATION_CAMERA ||
      _sdf.Type() == sdf::SensorType::WIDE_ANGLE_CAMERA)
  {
    auto cameraSensor = dynamic_cast<sensors::CameraSensor *>(sensor);
    this->cam_ptrs[sens_name] = cameraSensor;
    // store all camera  infos (even if not requested)
    auto *camera_sensor_sdf = _sdf.CameraSensor();
    if (camera_sensor_sdf == nullptr)
    {
      std::cout << "Warning: Camera sensor ptr doesn't exist for " << sens_name << "\n";
    }
    else
    {
      camera_info_data_map[sens_name] = gz::msgs::CameraInfo();
      camera_poses[sens_name] = _sdf.RawPose();
      // set h and w
      camera_info_data_map[sens_name].set_height(camera_sensor_sdf->ImageHeight());
      camera_info_data_map[sens_name].set_width(camera_sensor_sdf->ImageWidth());
      // set distortions
      auto *mutable_dist = camera_info_data_map[sens_name].mutable_distortion();
      mutable_dist->mutable_k()->Resize(5, 0.0);
      mutable_dist->set_k(0, camera_sensor_sdf->DistortionK1());
      mutable_dist->set_k(1, camera_sensor_sdf->DistortionK2());
      mutable_dist->set_k(2, camera_sensor_sdf->DistortionK3());
      mutable_dist->set_k(3, camera_sensor_sdf->DistortionP1());
      mutable_dist->set_k(4, camera_sensor_sdf->DistortionP2());
      // set intrinsics
      auto *mutable_intrinsic = camera_info_data_map[sens_name].mutable_intrinsics();
      mutable_intrinsic->mutable_k()->Resize(9, 0.0);
      mutable_intrinsic->set_k(0, camera_sensor_sdf->LensIntrinsicsFx());
      mutable_intrinsic->set_k(2, camera_sensor_sdf->LensIntrinsicsCx());
      mutable_intrinsic->set_k(4, camera_sensor_sdf->LensIntrinsicsFy());
      mutable_intrinsic->set_k(5, camera_sensor_sdf->LensIntrinsicsCy());
      mutable_intrinsic->set_k(8, 1.0);
      // set projections
      auto *mutable_projection = camera_info_data_map[sens_name].mutable_projection();
      mutable_projection->mutable_p()->Resize(12, 0.0);
      mutable_projection->set_p(0, camera_sensor_sdf->LensProjectionFx());
      mutable_projection->set_p(2, camera_sensor_sdf->LensProjectionCx());
      mutable_projection->set_p(3, camera_sensor_sdf->LensProjectionTx());
      mutable_projection->set_p(5, camera_sensor_sdf->LensProjectionFy());
      mutable_projection->set_p(6, camera_sensor_sdf->LensProjectionCy());
      mutable_projection->set_p(7, camera_sensor_sdf->LensProjectionTy());
      mutable_projection->set_p(10, 1.0);
    }
    // Add image binding callback here if in requested sensors
    // if (this->requested_sensor_map.find(sens_name) != this->requested_sensor_map.end() &&
    //     this->requested_sensor_map[sens_name])
    if (true) // Change in API, if sensors are enabled, then we collect all sensor data
    {
      if (_sdf.Type() == sdf::SensorType::DEPTH_CAMERA)
      {
        // Depth cam needs explicit casting
        auto depth_ptr = dynamic_cast<sensors::DepthCameraSensor *>(cameraSensor);
        if (depth_ptr)
        {
          this->conn_ptrs[sens_name] =
              depth_ptr->ConnectImageCallback(
                  std::bind(&Sensors::ImageSensorCB, this,
                            std::placeholders::_1, cameraSensor->Name()));
        }
      }
      else
      {
        this->conn_ptrs[sens_name] =
            cameraSensor->ConnectImageCallback(
                std::bind(&Sensors::ImageSensorCB, this,
                          std::placeholders::_1, cameraSensor->Name()));
      }
    }
  ...
  {
    auto gpuLidarSensor = dynamic_cast<sensors::GpuLidarSensor *>(sensor);
    this->gpu_lidar_ptrs[sens_name] = gpuLidarSensor;

    // Add lidar binding callback here if in requested sensors
    // if (this->requested_sensor_map.find(sens_name) != this->requested_sensor_map.end() &&
    //     this->requested_sensor_map[sens_name])
    if (true) // change in API
    {
      this->conn_ptrs[sens_name] =
          gpuLidarSensor->ConnectNewLidarFrame(
              std::bind(&Sensors::LidarSensorCB, this,
                        std::placeholders::_1, // scan data pointer
                        std::placeholders::_2, // width
                        std::placeholders::_3, // height
                        std::placeholders::_4, // channels
                        std::placeholders::_5, // subscriber
                        gpuLidarSensor->Name()));
      lidar_poses[sens_name] = _sdf.RawPose();
    }
  }
...
void Sensors::LidarSensorCB(const float *scan,
                            unsigned int width,
                            unsigned int height,
                            ...
                            std::string name)
{
  // Store pointer to GPU lidar data (non-owning pointer - data owned by sensor)
  // WARNING: This pointer is only valid until the next sensor update
  {
    std::lock_guard<std::mutex> l(lidar_mutex);
    LidarFrameView view{scan, width, height, channels, 0.f, 0.f, 0.f, 0.f};
    // Beam angles are required to turn the polar range image into Cartesian
    // points; pull them from the sensor (set from SDF).
    if (auto *s = this->gpu_lidar_ptrs[name])
    {
      view.angle_min = static_cast<float>(s->AngleMin().Radian());
      view.angle_max = static_cast<float>(s->AngleMax().Radian());
      view.vert_angle_min = static_cast<float>(s->VerticalAngleMin().Radian());
      view.vert_angle_max = static_cast<float>(s->VerticalAngleMax().Radian());
    }
    this->gpu_lidar_data_map[name] = view;

    // Call cbs
    if (lidar_cbs.find(name) != lidar_cbs.end() && lidar_cbs[name].size() != 0)
    {
      for (const auto &cb : lidar_cbs[name])
        cb(this->gpu_lidar_data_map[name]);
    }
  }
}

void Sensors::ImageSensorCB(const gz::msgs::Image &img, std::string name)
{
  // Copy image message to local storage
  // NOTE: This involves a full copy of image data
  {
    std::lock_guard<std::mutex> l(img_mutex);
    this->image_data_map[name] = img;

    // call cbs
    if (img_cbs.find(name) != img_cbs.end() && img_cbs[name].size() != 0)
    {
      for (const auto &cb : img_cbs[name])
        cb(img);
    }
  }
}
void  Sensors::StartCameraRecording(std::string cam_name, int height, int width, int fps,
    const math::Pose3d& initial_pose,
    std::string output_file){
      custom::VideoRecordingManager::CameraConfig cfg;
      cfg.width = width; cfg.height = height; cfg.fps = fps;
      this->dataPtr->videoRecorder.StartRecording(cam_name, initial_pose, output_file, cfg);
      // The render scene is initialized lazily, only when a rendering-sensor
      // component is found in the ECM (see PostUpdate). In a sensor-free
      // world that never happens, so the recorder would never get a scene.
      // Force initialization here, same as PostUpdate does.
      if (!this->dataPtr->initialized)
      {
        std::unique_lock<std::mutex> lock(this->dataPtr->renderMutex);
        gzdbg << "Initialization needed (camera recording)" << std::endl;
        this->dataPtr->doInit = true;
        this->dataPtr->renderCv.notify_one();
      }
    }
void  Sensors::UpdateCameraRecordingPose(std::string cam_name, const math::Pose3d& pose){
  this->dataPtr->videoRecorder.UpdateRecordingCamPos(cam_name, pose);
}
void  Sensors::StopCameraRecording(std::string cam_name){
    this->dataPtr->videoRecorder.StopRecording(cam_name);
}
bool Sensors::CameraRecordingQueued(const std::string& cam_name){
  return this->dataPtr->videoRecorder.RecordingState(cam_name) ==
      custom::VideoRecordingManager::State::kQueued;
}
bool Sensors::CameraRecordingBusy(const std::string& cam_name){
  return this->dataPtr->videoRecorder.RecordingState(cam_name) !=
      custom::VideoRecordingManager::State::kNone;
}


void Sensors::DepthImageSensorCB(const float *_data, unsigned int _width, unsigned int _height,
                                 unsigned int /*_channels*/, const std::string & /*_format*/, std::string name)
{
  gz::msgs::Image img;
  img.set_width(_width);
  img.set_height(_height);
  img.set_pixel_format_type(gz::msgs::PixelFormatType::R_FLOAT32);
  img.set_step(_width * sizeof(float)); // bytes per row
  img.set_data(_data, _width * _height * sizeof(float));
  {
    std::lock_guard<std::mutex> l(img_mutex);
    this->image_data_map[name] = img;
...
std::vector<std::string> Sensors::CameraSensorNames()
{
  std::vector<std::string> res;
  for (auto *s : this->GetSensors())
  {
    if (dynamic_cast<sensors::CameraSensor *>(s) ||
        dynamic_cast<sensors::DepthCameraSensor *>(s) ||
        dynamic_cast<sensors::RgbdCameraSensor *>(s) ||
        dynamic_cast<sensors::ThermalCameraSensor *>(s) ||
        dynamic_cast<sensors::BoundingBoxCameraSensor *>(s) ||
        dynamic_cast<sensors::SegmentationCameraSensor *>(s) ||
        dynamic_cast<sensors::WideAngleCameraSensor *>(s))
    {
...
std::vector<std::string> Sensors::LidarSensorNames()
{
  std::vector<std::string> res;
  for (auto *s : this->GetSensors())
  {
    if (dynamic_cast<sensors::GpuLidarSensor *>(s))
    {
      res.push_back(s->Name());
    }
  }
...
std::vector<sensors::Sensor *> Sensors::GetSensors()
{
