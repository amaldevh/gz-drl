 ...
 * Copyright (C) 2025 Amaldev Haridevan, York University
 * 
...
#include "plugins/ackermann_plugin.hh"
...
//   public: transport::Node::Publisher tfPub;
  ...
  // 
  public: double linVelRef;
  public: double angVelRef;
};
...
//   GZ_PROFILE("AckermannSteering::PreUpdate");
  ...
  if (!rightSteeringPos)
  {
   ...
   {
    auto velCmdComp = _ecm.Component<AckerMannVelocityCmdComp>(this->dataPtr->canonicalLink.Entity());
    if (velCmdComp) {
        this->dataPtr->linVelRef = velCmdComp->Data().X(); this->dataPtr->angVelRef = velCmdComp->Data().Y(); 
        velCmdComp->Data().X() = 0.0;velCmdComp->Data().Y() = 0.0;
        }
  }
...
//   GZ_PROFILE("AckermannSteering::UpdateVelocity");
  ...
  double linVel = this->linVelRef;
  double angVel = this->angVelRef;
