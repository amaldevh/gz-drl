...
from experiments_paper.benchmark_realtime_env.ros2_hover import HoverEnv
import time
...
def benchmark_n_envs(n_envs, n_steps, warmup, backend):
...
if __name__ == "__main__":
    ...
    if args.backend =="thread":
        raise RuntimeError("thread hangs ros envs")
    if not args.benchmark_dummy:
        fps, avg_cpu, avg_gpu, avg_ram = benchmark_n_envs(n_envs, n_steps, warmup, args.backend)
    else:
        fps,  avg_cpu, avg_gpu, avg_ram  = benchmark_n_dummy_envs(n_envs, n_steps, warmup)
