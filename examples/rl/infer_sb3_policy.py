...
def parse():
    ...
    parser.add_argument("-realtime", action="store_true", help="Run inference with realtime factor of 1.0")
...
if __name__ == "__main__":
    ...
    args = parse()
    rt_cb = lambda: time.sleep(0.6e-2) if args.realtime else None
    Adapter = VecAdapter
    args_ = ()
    log_dir = args.logdir
    os.makedirs(log_dir, exist_ok=True)
    
    # Request domain randomization. Its concrete effect and cadence are task-specific.
    ...
    obs = _eval_env.reset()
    while len(episode_rewards) < total_episodes:
    ...
    plt.title(f"Evaluation Rewards (DR=True) - {args.env_name}\n({alg_name})")
