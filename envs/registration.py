...
_TASK_HORIZON_ENVS = frozenset({
  "GazeboPoolHoverEnv-v0",
  "GazeboPoolInvertedPendulumLLEnv-v0",
  "GazeboPoolMultiAgentFormationLLEnv-v0",
  "GazeboPoolPayloadTransportEnv-v0",
  "GazeboPoolPayloadTransportLLEnv-v0",
  "GazeboPoolTrajectoryTrackingEnv-v0",
  "GazeboPoolTrajectoryTrackingLLEnv-v0",
})
...
class EnvRegistry:
  ...
  def make_spec(self, task_id: str, **make_kwargs: Any) -> Any:
    ...
    if task_id in _TASK_HORIZON_ENVS and "max_episode_steps" not in kwargs:
      defaults = spec_cls.gen_config()
      task_horizon = kwargs.get(
        "max_steps_per_episode", defaults.max_steps_per_episode
      )
      kwargs["max_episode_steps"] = task_horizon
