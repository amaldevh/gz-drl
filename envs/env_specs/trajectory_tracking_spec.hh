...
#ifndef TRAJECTORY_TRACKING_SPEC_HH_
#define TRAJECTORY_TRACKING_SPEC_HH_

#include "env_specs/gazebo_envpool_spec.hh"

/** NMPC-RRL-style trajectory tracking with geometric position control. */
class TrajectoryTrackingSpec : public GazeboSpec
{
public:
    ...
    static decltype(auto) DefaultConfig()
    {
        ...
        return ConcatDict(
            ...
            MakeDict(
                ...
                "action_scaling"_.Bind(
                    std::vector<float>{9.0f, 9.0f, 15.0f}),
                 "action_bias"_.Bind(
                    std::vector<float>{0.0f, 0.0f, 0.0f})));
    }

    /** Observation mirrors NMPC-RRL without privileged information.
     *
     * Rotation enabled:
     *   history = [z, velocity(3), rotation(9), omega(3), action(3)]
     *   future  = [position error(3), velocity error(3), desired rotation(9)]
     * Rotation disabled:
     *   history = [z, velocity(3), omega(3), action(3)]
     *   future  = [position error(3), velocity error(3)]
     */
    ...
    static decltype(auto) StateSpec(const Config &conf)
    {
        ...
        const int history_features = use_rotation ? 19 : 10;
...
using TrajectoryTrackingEnvSpec = EnvSpec<TrajectoryTrackingSpec>;

#endif // TRAJECTORY_TRACKING_SPEC_HH_
